...
export async function generateGeminiResponse(
  ...
  if (type === "open_ended") {
    prompt = `Generate ${amountOfQuestions} easy to moderat open-ended questions on the topic "${topic}" with the word limit of 25 words for both the question and the answer.
  Return the response as a JSON array with the structure :
  [
    {
      "question": "string",
